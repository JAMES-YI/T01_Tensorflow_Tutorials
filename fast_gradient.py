...
def fgm(model, x, eps=0.01, epochs=1, sign=True, clip_min=0., clip_max=1.):
    ...
    ybar = model(xadv) # distribution
    yshape = ybar.get_shape().as_list()
    ydim = yshape[1]

    indices = tf.argmax(ybar, axis=1) # true label
    target = tf.cond(
        tf.equal(ydim, 1),
    ...
    def _body(xadv, i):
        ...
        xadv = tf.stop_gradient(xadv + eps*noise_fn(dy_dx)) # update adversarial samples
    ...
    xadv, _ = tf.while_loop(_cond, _body, (xadv, 0), back_prop=False,
                            name='fast_gradient')
    '''JYI
    1 body will update the loop variables (used both in the body and condition)
    2 search only for one step, may or may not generate successful adversarial samples
    3 tf.while_loop will execute the loop and return the final results
    4 used as part of the whole graph session, and need to be called within a given graph
    5 target label is not specified
    '''
    return xadv
...
def fgmt(model, x, y=None, eps=0.01, epochs=1, sign=True, clip_min=0.,
    ...
    ybar = model(xadv) # probability distribution
    ydim = ybar.get_shape().as_list()[1]
    n = tf.shape(ybar)[0]

    if y is None:
        indices = tf.argmin(ybar, axis=1) # generate adversarial sample to have the least likely label
    else:
    ...
    xadv, _ = tf.while_loop(_cond, _body, (xadv, 0), back_prop=False,
                            name='fast_gradient_target') # loop only update once
    return xadv
