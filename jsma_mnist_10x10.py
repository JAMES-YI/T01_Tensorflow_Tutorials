...
def model(x, logits=False, training=False):
    '''JYI
    model(env.x, logits=True, training=env.training)
    env.training = False
...
if __name__=='__main__':
    ...
    print('\nLoading MNIST, splitting and shuffling data')

    mnist = tf.keras.datasets.mnist
    (X_train, y_train), (X_test, y_test) = mnist.load_data()
    print('X_train: max {}, min {}, shape {}\n'.format(np.max(X_train[0]),np.min(X_train[0]),np.array(X_train[0]).shape)) # [0,255],
    print('Y_train: {}\n'.format(y_train.shape)) # (60000,1)
    ...
    3 tf.keras.datasets.mnist
    4 pass in class, indicating that nothing happens
    """
    ...
    with tf.variable_scope('model'):
        ...
        env.ybar, logits = model(env.x, logits=True, training=env.training) # function call; y_bar, is the probability
...
'''JYI
1 tf.placeholder_with_default(False, (), name='mode')
2 what DNN is attacked, model()
3 how the DNN is trained
